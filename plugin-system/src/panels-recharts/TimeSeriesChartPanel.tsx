...
export function TimeSeriesChartPanel({
  ...
  // Default to bottom legend (shadcn ChartLegend); set legend.position: 'hidden' to disable.
  // ViewPanelDialog forces legend.position: 'right' when a chart is expanded.
  const showLegend = chartSpec.legend?.position !== 'hidden' && seriesKeys.length > 0;
  const legendPosition =
    chartSpec.legend?.position === 'top' || chartSpec.legend?.position === 'right'
      ? chartSpec.legend.position
      : 'bottom';
