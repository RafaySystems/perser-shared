...
import { parseVariables, replaceVariables } from '@rafaysystems/components';
...
export const LocalPrometheusLabelValuesVariable: VariablePlugin<LabelValuesSpec> = {
  getVariableOptions: async (spec, ctx) => {
    ...
    const { data } = await promClient.labelValues({
    ...
    return {
      data: data.map((value: string) => ({ label: value, value })),
    };
  },
  dependsOn: (spec) => ({
    variables: [
